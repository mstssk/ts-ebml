            ...
            switch (_a.label) {
                ...
                case 1:
                    ...
                    return [4 /*yield*/, (navigator.mediaDevices
                            .getUserMedia instanceof Function
                            ? navigator.mediaDevices.getUserMedia({ video: true, audio: true })
                            : navigator.getUserMedia instanceof Function
                                ? new Promise(function (resolve, reject) {
                                    return navigator.getUserMedia({ video: true, audio: true }, resolve, reject);
                                })
                                : navigator["webkitGetUserMedia"] instanceof Function
                                    ? new Promise(function (resolve, reject) {
                                        return navigator["webkitGetUserMedia"]({ video: true, audio: true }, resolve, reject);
                                    })
                                    : Promise.reject(new Error("cannot use usermedia")))];
                case 2:
                    stream = _a.sent();
                    if (logging) {
                        stream.addEventListener("active", function (ev) {
                            console.log(ev.type);
                        });
                        stream.addEventListener("inactive", function (ev) {
                            console.log(ev.type);
                        });
                        stream.addEventListener("addtrack", function (ev) {
                            console.log(ev.type);
                        });
                        stream.addEventListener("removetrack", function (ev) {
                            console.log(ev.type);
                        });
                    }
                    ...
                    if (logging) {
                        ...
                        rec.addEventListener("pause", function (ev) {
                            console.log(ev.type);
                        });
                        rec.addEventListener("resume", function (ev) {
                            console.log(ev.type);
                        });
                        rec.addEventListener("start", function (ev) {
                            console.log(ev.type);
                        });
                        rec.addEventListener("stop", function (ev) {
                            console.log(ev.type);
                        });
                    ...
                    ms = new MediaSource();
                    if (logging) {
                        ms.addEventListener("sourceopen", function (ev) {
                            console.log(ev.type);
                        });
                        ms.addEventListener("sourceended", function (ev) {
                            console.log(ev.type);
                        });
                        ms.addEventListener("sourceclose", function (ev) {
                            console.log(ev.type);
                        });
                        ms.sourceBuffers.addEventListener("addsourcebuffer", function (ev) {
                            console.log(ev.type);
                        });
                        ms.sourceBuffers.addEventListener("removesourcebuffer", function (ev) {
                            console.log(ev.type);
                        });
                    }
                    video = document.createElement("video");
                    if (logging) {
                        video.addEventListener("loadstart", function (ev) {
                            console.log(ev.type);
                        });
                        ...
                        video.addEventListener("waiting", function (ev) {
                            console.log(ev.type);
                        });
                        ...
                        video.addEventListener("emptied", function (ev) {
                            console.log(ev.type);
                        });
                        video.addEventListener("stalled", function (ev) {
                            console.log(ev.type);
                        });
                        ...
                        video.addEventListener("ratechange", function (ev) {
                            console.log(ev.type);
                        });
                    ...
                    return [4 /*yield*/, new Promise(function (resolve) {
                            ms.addEventListener("sourceopen", function () { return resolve(); }, { once: true });
                        })];
                case 3:
                    _a.sent();
                    sb = ms.addSourceBuffer(rec.mimeType);
                    if (logging) {
                        sb.addEventListener("updatestart", function (ev) {
                            console.log(ev.type);
                        }); // annoying
                        sb.addEventListener("update", function (ev) {
                            console.log(ev.type);
                        }); // annoying
                        sb.addEventListener("updateend", function (ev) {
                            console.log(ev.type);
                        }); // annoying
                        sb.addEventListener("error", function (ev) {
                            console.error(ev.type, ev);
                        });
                        sb.addEventListener("abort", function (ev) {
                            console.log(ev.type);
                        });
                    }
                    aborted = false;
